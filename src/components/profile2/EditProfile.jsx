    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get(
          `htttp://13.232.209.194/users/${userFromRedux.id}`,
          { headers }
        );
        ...
        if (role === 'founder') {
          try {
            const founderResponse = await axios.get(
              `htttp://13.232.209.194/users/entrepreneurs/${userData._id}`,
              { headers }
            );
          ...
          try {
            const investorResponse = await axios.get(
              `htttp://13.232.209.194/users/investors/${userData._id}`,
              { headers }
            );
    ...
    try {
      ...
      if (hasUserDetailsChanged) {
        try {
          ...
          await axios.put(
            `htttp://13.232.209.194/users/${user._id}`,
            userDetails,
            { headers }
          );
          setOriginalUserDetails(JSON.stringify(userDetails));
      ...
      if (hasRoleDetailsChanged) {
        try {
          if (formData.role === 'founder') {
            ...
            await axios.put(
              `htttp://13.232.209.194/users/entrepreneurs/${user._id}`,
            ...
            await axios.put(
              `htttp://13.232.209.194/users/investors/${user._id}`,
